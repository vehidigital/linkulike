...
import { Edit3, Trash2, GripVertical, Globe, ExternalLink, Eye, EyeOff, Palette, RotateCcw, Phone, Mail, Link as LucideLink, ArrowRight, BarChart } from "lucide-react";
              ...
              <div className="flex items-center">
                <div className="flex items-center space-x-3 flex-1 min-w-0">
                  ...
                  <div 
                    ...
                    style={{
                      backgroundColor: getLinkButtonColors(link).backgroundColor,
                      color: getLinkButtonColors(link).textColor
                    }}
                  >
...
function EditLinkForm({ link, onSave, onCancel, theme, t }: EditLinkFormProps) {
  ...
  // Hilfsfunktion, um zu erkennen, ob ein Wert ein Emoji ist
  function isEmoji(str: string) {
    return /\p{Emoji}/u.test(str);
  }

  // HTTPS-Validierung und Live-Check
  useEffect(() => {
    if (!formData.url) {
      setUrlError(null);
      setIsUrlValid(false);
    ...
    if (!formData.url.startsWith('https://')) {
      setUrlError(t.onlyHttps || 'Nur https:// Links erlaubt');
      setIsUrlValid(false);
      return;
    }
    // Live-Check, ob die URL erreichbar ist
    setIsCheckingUrl(true);
    setUrlError(null);
    setIsUrlValid(false);
    fetch(formData.url, { method: 'HEAD', mode: 'no-cors' })
      .then(() => {
        setUrlError(null);
        setIsUrlValid(true);
      })
      .catch(() => {
        setUrlError(t.urlNotReachable || 'URL ist nicht erreichbar');
        setIsUrlValid(false);
      })
  ...
  const LUCIDE_ICONS = [
    { name: "Telefon", value: "lucide-phone", icon: <Phone className="w-5 h-5" /> },
    { name: "E-Mail", value: "lucide-mail", icon: <Mail className="w-5 h-5" /> },
    { name: "Link", value: "lucide-link", icon: <LucideLink className="w-5 h-5" /> },
    { name: "Pfeil", value: "lucide-arrow-right", icon: <ArrowRight className="w-5 h-5" /> },
    { name: "Graf", value: "lucide-bar-chart", icon: <BarChart className="w-5 h-5" /> },
    { name: "Globus", value: "lucide-globe", icon: <Globe className="w-5 h-5" /> },
  ];
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        />
        {isCheckingUrl && (
          <div className="text-xs text-gray-400 mt-1">{t.checkingUrl || 'Prüfe URL...'}</div>
        )}
        {urlError && (
      ...
      <div>
        ...
        <div className="flex gap-2 mb-2">
          {/* Icon wählen Button */}
          <Button type="button" variant="outline" size="sm" onClick={() => { setShowEmojiPicker(false); setShowIconPicker(v => !v); }}>
            {LUCIDE_ICONS.some(i => i.value === formData.icon)
              ? LUCIDE_ICONS.find(i => i.value === formData.icon)?.icon
              : <span className="text-gray-400">+</span>}
            <span className="ml-2">{t.chooseIcon || "Icon wählen"}</span>
          </Button>
          {/* Emoji wählen Button */}
          <Button type="button" variant="outline" size="sm" onClick={() => { setShowIconPicker(false); setShowEmojiPicker(v => !v); }}>
            {typeof formData.icon === 'string' && formData.icon.length <= 2 && !LUCIDE_ICONS.some(i => i.value === formData.icon)
              ? formData.icon
              : <span className="text-gray-400">😊</span>}
            <span className="ml-2">{t.chooseEmoji || "Emoji wählen"}</span>
          </Button>
        </div>
        {/* Icon Picker Overlay: Nur Lucide-Icons */}
        {showIconPicker && (
          <div className="absolute z-10 bg-white border rounded shadow p-2 grid grid-cols-6 gap-2 max-w-lg max-h-60 overflow-y-auto mt-2">
            {LUCIDE_ICONS.map((item) => (
              <button
                key={item.value}
                type="button"
                onClick={() => { setFormData({ ...formData, icon: item.value }); setShowIconPicker(false); }}
                className={`p-2 rounded border flex items-center justify-center hover:bg-gray-50 ${formData.icon === item.value ? "border-blue-500 bg-blue-50" : "border-gray-200"}`}
                title={item.name}
        ...
        {/* Emoji Picker Overlay: Nur Emojis */}
        {showEmojiPicker && (
          <div className="absolute z-10 bg-white border rounded shadow p-2 grid grid-cols-8 gap-1 max-w-xs max-h-40 overflow-y-auto mt-2">
            {EMOJI_LIST.map((emoji) => (
              <button
                key={emoji}
                type="button"
                className={`text-xl p-1 rounded hover:bg-gray-100 ${formData.icon === emoji ? "ring-2 ring-blue-500" : ""}`}
        ...
        <div className="mb-2">
          ...
          <div className="max-w-xs truncate overflow-x-auto" title={formData.title}>
            <SolidLinkButton
              title={formData.title}
              color={currentColor}
              icon={LUCIDE_ICONS.some(i => i.value === formData.icon)
                ? LUCIDE_ICONS.find(i => i.value === formData.icon)?.icon
                : (typeof formData.icon === 'string' && formData.icon.length <= 2 ? formData.icon : undefined)}
      ...
      <div className="flex justify-end space-x-2">
        ...
        <Button type="submit" disabled={!isUrlValid || isCheckingUrl}>{t.saveChanges || "Zur Vorschau hinzufügen"}</Button>
      </div>
    </form>
  );
} 